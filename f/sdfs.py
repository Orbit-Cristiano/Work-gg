...
def reccurent_find_value(tests: dict, test_id: int, value_set):
    for test in tests:
        if test['id'] == test_id:
            ...
            return
        if 'values' in test:
            if reccurent_find_value(test['values'], test_id, value_set):
                return True
    return False
    ...
    for item in values_json['values']:
        value_test_id = item['id']
        if reccurent_find_value(tests_json['tests'], value_test_id, item['value']):
            print(f"Value updated for test with id {value_test_id}")
